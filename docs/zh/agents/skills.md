...
如果 Skill 项目被共享给协作者，项目型 Skill 也按项目访问权限共享：协作者会在 **团队管理 → Skill** 中看到同一个项目型 Skill，并可以装配到自己的 Agent。`viewer` 可以使用最近一次缓存；`editor` 和项目 owner 可以在 Skill 项目中更新缓存，更新后所有协作者引用同一份新缓存。协作者各自的 Agent、对话和批注仍然按用户隔离。
...
推荐用法是让 Agent 先创建拆分后的参考文件，再由你人工检查内容。Agent 执行时会在消息标题旁显示本轮 `读文件 X · 写文件 Y`，如果它只是口头说“我会创建文件”但没有写入计数，说明它没有真正调用项目写入工具。确认这些 reference 已经适合运行时使用后，进入 **版本 → 项目大版本 → Skill 缓存** 点击 **更新 Skill 缓存**。没有更新缓存前，已装配这个 Skill 的 Agent 仍然使用上一次缓存内容。
