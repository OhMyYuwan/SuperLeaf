...
> 前置知识：Nanobot 是一个独立的 Python 进程，它自己不直接跑大模型，而是再调用上游 LLM（OpenRouter、OpenAI、Claude、DeepSeek 等）。YuwanLabWriter 只通过 Nanobot 的 **OpenAI 兼容 HTTP API** 和它说话，不关心它背后用的哪家模型。

## 1. 架构速记

```
YuwanLabWriter 前端 ── 选中文字 / 指令 ──▶ YuwanLabWriter 后端 (FastAPI)
                                                     │
...
- **上游 LLM key**（如 OpenRouter）配置在 Nanobot 自己的 `~/.nanobot/config.json`，不要填进 YuwanLabWriter。
- YuwanLabWriter 里填的 "API Key" 只是保护 Nanobot HTTP 端口的令牌；单机自用可以直接写 `dummy`。
...
## 5. 在 YuwanLabWriter 里注册这个 Nanobot

1. 启动 YuwanLabWriter（`./start.sh` 或手动）。
...
假设拿到 `192.168.1.23`。其他设备上浏览器打开 YuwanLabWriter，注册 provider 时用：
...
在 YuwanLabWriter 里各注册一条 provider：
...
第一阶段 YuwanLabWriter 一次只会用激活的那条，切换时在设置里把另一条点「激活」即可。后续做多 Agent 编排时，这两条会被当成可以并行调用的对象。
...
**想换 Nanobot 的上游模型**
- 改 `~/.nanobot/config.json` 的 `agents.defaults.model` → 重启 `nanobot serve` → 回 YuwanLabWriter 再点一次测连。
