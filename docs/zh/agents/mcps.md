...
Docker 用户版通过 `deploy/.env` 控制本地开关：
...
1. 打开 **团队管理 → MCP**。
2. 点击 **自定义 MCP**。
3. 切换到 **Local Trusted stdio** 子标签页。
4. 填写 `Owner`、`MCP 名称`、`Command`、`Args`、`Env` 和 `Allowed tools`。
5. 保存后在 **拥有的 MCP** 中运行连通性检查，再回到 Agent 配置里勾选使用。

`deploy/.env` 是本机配置文件，默认被 git 忽略。不要把 `YLW_MCP_STDIO_ENABLED=true` 作为公开发行默认值。

### 本地开发进程中开启

如果你不是用 Docker，而是自己启动 backend 进程，需要在启动前设置环境变量：
...
```bash
cd services/backend
export YLW_MCP_STDIO_ENABLED=true
...
| 子标签页 | 适用场景 | 默认状态 |
|---|---|---|
| Remote Endpoint | 公开版、多人部署、用户自己的远程 MCP 服务 | 可用 |
| Local Trusted stdio | 本机调试、单用户可信部署、后端本机执行 stdio MCP | 需要 `YLW_MCP_STDIO_ENABLED=true` |
