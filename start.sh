...
start_local_agent_host() {
  ...
  if command -v screen >/dev/null 2>&1; then
    ...
    local dir_q logfile_q bind_q port_q origins_q nanobot_url_q
    printf -v dir_q "%q" "$LOCAL_AGENT_HOST_DIR"
    printf -v logfile_q "%q" "$logfile"
    printf -v bind_q "%q" "$LOCAL_AGENT_HOST_BIND"
    printf -v port_q "%q" "$LOCAL_AGENT_HOST_PORT"
    printf -v origins_q "%q" "${SL_LOCAL_AGENT_HOST_ORIGINS:-*}"
    printf -v nanobot_url_q "%q" "$LOCAL_AGENT_HOST_NANOBOT_URL"
    screen -S "$screen_name" -X quit >/dev/null 2>&1 || true
    screen -dmS "$screen_name" bash -lc \
      "cd $dir_q && SL_LOCAL_AGENT_HOST_BIND=$bind_q SL_LOCAL_AGENT_HOST_PORT=$port_q SL_LOCAL_AGENT_HOST_ORIGINS=$origins_q SL_LOCAL_AGENT_HOST_NANOBOT_URL=$nanobot_url_q exec node server.mjs >> $logfile_q 2>&1"
  else
    (
      cd "$LOCAL_AGENT_HOST_DIR"
      SL_LOCAL_AGENT_HOST_BIND="$LOCAL_AGENT_HOST_BIND" \
        SL_LOCAL_AGENT_HOST_PORT="$LOCAL_AGENT_HOST_PORT" \
        SL_LOCAL_AGENT_HOST_ORIGINS="${SL_LOCAL_AGENT_HOST_ORIGINS:-*}" \
