...
| 任务 | 先读 |
|---|---|
| Agent / Skill / MCP 面板 | 普通 Agent/Skill/MCP 先读 `LOAD_RULES.native-agent-skills`，再看 `team-tab/`（容器 `team-tab/index.tsx`，面板拆在 `providers.tsx`、`McpManagementPanel.tsx`、`SkillManagementPanel.tsx` 等）、`nativeAgentStore.ts`、`native_agents.py` |
