...
| MCP | Agent 可按需调用的外部工具服务，先添加到拥有的 MCP，再授权给 Agent |
...
- Agent 可按需启用 MCP 工具，只有用户授权的 MCP 会进入可调用工具集。
...
{: .important }
Agent 运行时只能读取你勾选的 Skill，并只能调用你给该 Agent 启用的 MCP。没有勾选或启用的能力不会进入上下文/工具集。
...
- Skill 的可见性与 Agent 独立，详见 [Skill 使用与市场](skills.md)。
- MCP 的市场发现与 Agent 授权是两步，详见 [MCP 使用与市场](mcps.md)。

## 下一步

- [Skill 使用与市场](skills.md)
- [MCP 使用与市场](mcps.md)
