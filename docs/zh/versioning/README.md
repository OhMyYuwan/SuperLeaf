...
如果当前项目是 **Skill 项目**，**版本 → 项目大版本** 顶部会额外显示 **Skill 缓存** 区域。

这里的 **更新 Skill 缓存** 不等同于保存项目大版本：

| 动作 | 面向对象 | 结果 |
|---|---|---|
| 更新 Skill 缓存 | Agent runtime | 把当前项目文件树写入 `skills-cache`，让装配同一 Skill 的 Agent 使用最新版 |
| 保存大版本 | 人类版本管理 | 把整个项目树提交为服务器端归档 commit，用于对比、恢复和下载 |

Skill 项目仍然可以正常保存大版本。推荐流程是：先在项目里编辑和测试 `SKILL.md`，确认要给 Agent 使用时点击 **更新 Skill 缓存**；当这个开发状态值得长期保留时，再保存一个项目大版本。
