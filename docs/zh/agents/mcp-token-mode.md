...
{: .important }
普通 `./start.sh backend` 默认不会挂载 `/mcp`。Backend MCP 是可选 profile，必须用 `./start.sh mcp` 或 `YLW_MCP_SERVER_ENABLED=1 ./start.sh backend` 显式开启。这样它和 Local Agent Host 的浏览器 Bridge 生命周期保持分离。
...
3. 点击 **创建 MCP Token**，填写：
   ...
   - **作用域**：
     ...
     - `write`：额外允许 `propose_doc_edit`、`create_suggestion`、`project_write_text_file`、`project_create_text_file`。
...
| 工具调用静默失败 / 连不上后端 | 运行 `./start.sh mcp`，并确认 `http://127.0.0.1:8000/mcp` 可达 |
