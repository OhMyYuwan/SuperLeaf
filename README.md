...
- **🔒 隐私优先** - Provider key、GitHub token、原生 Agent 凭证和 Skill 内容加密存储；私有 Agent 资产按用户隔离，项目型 Skill 按项目成员权限共享
...
除了上传单个 `SKILL.md` 或从 Skill Market 安装，SuperLeaf 也支持把 Skill 作为 **Skill 项目** 维护：
...
3. 明确要求创建 reference 或 example 时，原生 Agent 可以在当前项目数据库树里新增文本文件；它不会获得服务器磁盘任意写权限，也不会覆盖已有同名文件。
4. 在 **版本 → 项目大版本** 顶部点击 **更新 Skill 缓存**，把当前项目文件树写入本地 Skill cache。
5. 进入 **团队管理 → Skill**，项目型 Skill 会出现在本地 Skill 库；点击标题可直接回到对应 Skill 项目。
6. 在 Agent 表单中勾选这个 Skill 后，Agent 运行时会读取最近一次缓存的 `SKILL.md` 和安全文本辅助文件。

从 Skill Market 安装的标准 Skill 仍然是只读版本；点击 **复制到本地** 会把它转换成一个新的 Skill 项目，复制出的 `README.md` / `SKILL.md` 存在项目数据库树中，可以像普通项目一样编辑、归档和更新 Skill 缓存。

Skill cache 存在后端数据目录下的 `skills-cache`，供所有装配同一项目型 Skill 的 Agent 复用。编辑项目不会自动改变运行时 Skill，只有点击 **更新 Skill 缓存** 才会发布新的本地开发版本。

Skill 项目共享给协作者后，项目型 Skill 也按项目权限共享：`viewer` 可以在 **团队管理 → Skill** 中看到并装配最近一次 cache，`editor` 和 owner 可以在项目中更新同一份 cache。团队管理里移除项目型 Skill 只会卸载生成的本地/cache 入口并解绑 Agent，不会删除源 Skill 项目；重新进入项目更新缓存即可加载回来。

项目列表可以在账号设置中切换为 **Paper / Skill 分区** 或 **混合列表**，适合把论文项目和 Skill 开发项目分开查看。
...
- **在线状态**: 实时显示协作者头像和光标
- **通知系统**: SSE 事件流推送项目更新
- **隐私隔离**: Agent 工作流和批注属于个人，不与协作者共享；共享 Skill 项目生成的项目型 Skill 按项目权限共享
